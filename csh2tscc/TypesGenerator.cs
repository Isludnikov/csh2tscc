...
public class TypesGenerator(TypesGeneratorParameters parameters)
{
    ...
    internal string BuildFileFromType(Type typeToWrite)
    {
        ...
        sb.AppendDebugLine(TypeScriptConstants.Name);
    ...
    private bool IsExportableType(Type type) =>
        ...
        !ExcludedType(type.FullName)) || TypeHasExportAttribute(type);
    ...
    private bool TypeHasExportAttribute(Type type)
    {
        ...
        var attributes = type.GetCustomAttributes(true).Where(x => !x.GetType().Namespace?.StartsWith("System") ?? true).ToArray();
